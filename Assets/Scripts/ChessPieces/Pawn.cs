        ...
        if (currentY != 0 && currentY != tileCountY - 1)
        {
            ...
            if (currentX != tileCountX - 1)
            {
                if (board[currentX + 1, currentY + direction] != null &&
                    ...
                    board[currentX + 1, currentY + direction].type != ChessPieceType.Portal)
                    ...
                    for (var y = 0; y < tileCountY; y++)
                        if (x != currentX + 1 || y != currentY + direction)
                            if (board[x, y]?.type == ChessPieceType.Portal)
                                if (y != 0 && y != tileCountY - 1)
                                    if (x != tileCountX - 1)
                                        if (board[x + 1, y + direction] != null &&
                                            board[x + 1, y + direction].team != team &&
                                            board[x + 1, y + direction].type != ChessPieceType.Portal)
                ...
                if (Mathf.Abs(lastMove[0].y - lastMove[1].y) == 2) // if last move was 2 tiles
                    if (board[lastMove[1].x, lastMove[1].y].team != team)
                    {
                        ...
                        if (currentY != 0 && currentY != tileCountY - 1)
                        {
                            if (currentX != tileCountX - 1)
                                if (board[currentX + 1, currentY + direction] != null)
                                    if (board[currentX + 1, currentY + direction]?.type ==
                                        ChessPieceType.Portal) // up right
                                        for (var x = 0; x < tileCountX; x++)
                                        for (var y = 0; y < tileCountY; y++)
                                            if (x != currentX + 1 || y != currentY + direction)
                                                if (board[x, y]?.type == ChessPieceType.Portal)
                                                    if (y != 0 && y != tileCountY - 1)
                                                        if (x != tileCountX - 1)
                                                            if (y == lastMove[1]
                                                                .y) //if portal and pawn are on the same y
                                                            {
                                                                availableMoves.Add(new Vector2Int(x + 1,
                                                                    y + direction));
                                                                return SpecialMove.EnPassant;
                                                            }
                                                    ...
                                                    if (y != 0 && y != tileCountY - 1)
                                                        if (x != 0)
                                                            if (y == lastMove[1]
                                                                .y) //if portal and pawn are on the same y
                                                            {
                                                                availableMoves.Add(new Vector2Int(currentY - 1,
                                                                    y + direction));
                                                                return SpecialMove.EnPassant;
                                                            }
                        }
